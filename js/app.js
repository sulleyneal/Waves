...
// Orchestration + rendering. Pulls each data source independently and renders
// whatever arrives, so a single failing API never blanks the page.

import { BEACHES, BEACH_FLAGS } from "./config.js";
import {
  fetchWeather,
  fetchMarine,
  fetchTides,
...
const degToCompass = (d) =>
  d == null ? "—" : COMPASS[Math.round(d / 22.5) % 16];

// ---------------------------------------------------------------------------
// Rip current risk.
// Prefer an OFFICIAL NWS Rip Current Statement when one is active. Otherwise
// fall back to a transparent estimate from surf + onshore wind, clearly
// labeled as an estimate so it is never mistaken for an official forecast.
// ---------------------------------------------------------------------------
function ripCurrentRisk(alerts, marine, weather) {
  const official = alerts.find((a) =>
    /rip current/i.test(a.event || "")
  );
  if (official) {
    return { level: "HIGH", label: "High", official: true, note: official.headline };
  }
  ...
  // Heuristic score. Longer-period, taller surf and stronger wind raise risk.
  let score = 0;
  if (waveFt != null) {
    if (waveFt >= 4) score += 3;
...
function uvAdvice(uv) {
  ...
  if (uv < 6) return ["Moderate", "SPF 30+, hat, shade midday.", "#eab308"];
  if (uv < 8) return ["High", "Reapply SPF often. Seek shade 10am–4pm.", "#f97316"];
...
function setLoading(on) {
  $("#dashboard").classList.toggle("loading", on);
...
function renderConditions(weather, water) {
  ...
  grid.innerHTML = "";
  if (!c) {
    grid.appendChild(el("p", "muted", "Weather data unavailable right now."));
    return;
  }
...
function renderAlerts(alerts) {
  ...
  if (!alerts.length) {
    heading.textContent = "No active warnings";
    wrap.appendChild(
      el("p", "muted", "✅ No active NWS alerts for this location right now.")
    );
    return;
  }
  heading.textContent = `${alerts.length} active warning${alerts.length > 1 ? "s" : ""}`;
  for (const a of alerts) {
    ...
    const meta = el("div", "alert-meta",
      `${a.severity || "Unknown"} severity · ${a.senderName || "NWS"}`);
...
function renderTides(tides) {
  ...
  for (const t of upcoming) {
    ...
    const time = new Date(t.ts).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" });
    row.appendChild(el("span", "tide-time", time));
...
function renderFlags() {
  const wrap = $("#flags");
  if (wrap.childElementCount) return; // static, render once
...
// --- Geolocation: snap to nearest preset beach ------------------------------
...
function useMyLocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      const b = nearestBeach(pos.coords.latitude, pos.coords.longitude);
      $("#beach-select").value = b.id;
      loadBeach(b);
      btn.textContent = "📍 Near me";
    },
    () => {
      btn.textContent = "📍 Near me";
      alert("Couldn't get your location. Pick a beach from the list instead.");
    },
    { timeout: 8000 }
  );
}
...
function init() {
  ...
  $("#beach-select").addEventListener("change", (e) => {
    const b = BEACHES.find((x) => x.id === e.target.value);
    if (b) loadBeach(b);
  });
  $("#refresh-btn").addEventListener("click", () => activeBeach && loadBeach(activeBeach));
  $("#geo-btn").addEventListener("click", useMyLocation);

  // Default to the first beach.
  loadBeach(BEACHES[0]);
}
